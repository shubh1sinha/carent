...
error_reporting(0);
  ...
  $res=mysqli_fetch_array($row);
      if(isset($_POST['submit'])){
        ...
        $query = "INSERT INTO `booked_vehicle`(`vehicle_id`, `model`, `start_date`, `days`, `username`) VALUES ('$vehicle_id','$model','$start_date','$days','$username')";
        ...
        if($query_run){
            ...
            echo 'alert("Booking Done Successfully!")</script>';
...
?>
<!-- This page is only accesible to user and he will rent car by giving input date and no of days-->
<!DOCTYPE html>
<html lang="en">
<head>
<!-- basic -->
...
<?php if($_SESSION['type']=='agency'){?>
        <title>caRent || User page</title>
<!-- if user logged in header will be My booked cars-->
<?php } 

else{?>
  <title>caRent || Rent Your Vehcile</title>
<?php } ?>
...
<form method="post" action="" style="padding: 20px; border: 2px solid yellow;">
  <div class="form-row">
    <div class="form-group col-md-6">
      ...
      <!-- Readble Value No changes can be done-->
      <input type="text" class="form-control"  value="<?php echo $res['model'];?>" name="model" readonly>
    </div>
    <div class="form-group col-md-6">
      <label for="inputPassword4">Vehicle Number</label>
      <!-- Readble Value No changes can be done-->
      <input type="text" class="form-control"  value="<?php echo $res['vehicle_number'];?>" name="vehicle_number" readonly>
    </div>
   <div class="form-group col-md-6">
      <label for="inputEmail4">Seating Capacity</label>
      <!-- Readble Value No changes can be done-->
      <input type="number" class="form-control"  value="<?php echo $res['capacity'];?>" name="capacity" readonly>
    </div>
    <div class="form-group col-md-6">
      <label for="inputPassword4">Rent Per Day (in Rs.)</label>
      <!-- Readble Value No changes can be done-->
      <input type="number" class="form-control" value="<?php echo $res['rent'];?>"  name="rent" readonly>
    </div>

        <div class="form-group col-md-6">
      <label for="inputEmail4">Start Date</label>
      <!--  changes can be done-->
      <input type="date" class="form-control"  value="<?php echo $res['model'];?>" name="start_date"required>
    </div>
    <div class="form-group col-md-6">
      <!-- Changes can be done-->
      <label>Totl Number of Days</label><br>
...
<center>
    <button type="submit" class="btn btn-warning" style="width:20%" name="submit">Book Now
  </button>
 </center>
</form>
</div>
